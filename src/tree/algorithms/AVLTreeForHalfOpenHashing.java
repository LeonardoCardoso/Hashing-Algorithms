...
public class AVLTreeForHalfOpenHashing {
		...
		if (currentNode == null) {
			...
			if (newNode.key < currentNode.key) {
				...
				if (currentNode.left == null) {
					...
					return insertAVL(currentNode.left, newNode);
				...
				if (newNode.key != currentNode.key) {
					...
					if (currentNode.right == null) {
						...
						return insertAVL(currentNode.right, newNode);
				...
				if (newNode.key == currentNode.key) {
					...
					HashPrinter.alreadyExistsInTree(newNode.key);
	...
	private void removeAVL(TreeNode startingNode, int searchingKey, int hash) {
		...
		if (startingNode == null) {
			...
			HashPrinter.notFound(searchingKey);
			...
			if (startingNode.key > searchingKey) {
				...
				HashPrinter.remotionSuccessful(searchingKey, searchingKey, startingNode, hash);
	...
	private void getAll(TreeNode node) {
		...
		HashPrinter.treeNode(node, l, r, p);
